...
fdeck <- NULL
tcount <-
  0 #true count based on 8 & 9 removal from the shoe to gain DRAGON advantage.
...
createDeck <- function(totalNumOfDecks = 2)
{
  ...
  while (numOfDecks <= totalNumOfDecks) {
    for (i in suits) {
      for (j in cards) {
        deck <-
          rbind.data.frame(deck, cbind.data.frame(j, i, values[match(j, cards)]))
      }
    }
...
StartLiveGame <- function(s = NewBaccShoe(),
                          dv = TRUE,
                          w = 25) {
  plyr <- NULL
  bnkr <- NULL
  ...
  while (length(s) > 7) {
    if (length(s) < 12) {
      if (dv) {
        #cat("\n")
        print(matrix(board, length(board) / 2, 2, byrow = TRUE))
        print(table(matrix(board, length(board) / 2, 2, byrow = TRUE)[, 1]))
  ...
  if (dv) {
    cat("Total Deals: ", length(board) / 2, "\n")
    print(table(matrix(board, (
      length(board) / 2
    ), 2, byrow = TRUE)[, 1]))
  }
  mboard <- matrix(board, (length(board) / 2), 2, byrow = TRUE)
...
simulplay <- function(s = NewBaccShoe(), n = 20) {
  ...
  for (i in 1:n) {
    ...
    cleanboard <- c(cleanboard, r_nties$length)
    credits <- bettingsys(r_nties$length)
  ...
  cnames <-
    c("Dragon", "Panda", "Tie", "Player", "Banker", "BRun", "PRun", "Credits")
  mresult <- matrix(dcount, n, 8, 1)
...
# Progressive Betting: Increase by 1 unit per loss and a max of 3 losses. Betting opposite of
# every outcome

bettingsys <- function(gseq) {
  credits <- 0
  ...
  for (i in 2:length(gseq)) {
    # cat(sprintf("PSEQ: %d",pseq))
    if (gseq[i] > 3) {
      credits <- credits + pseq - 6
      newseq <- 0
    } else if (gseq[i] > 1) {
      newseq <- gseq[i] - 1
      credits <- credits - (newseq * (newseq + 1) / 2) + pseq
      # print(credits)
      # invisible(readline("...."))
    } else {
      credits <- credits + pseq
    }
    if (gseq[i] > 3) {
      pseq <- 0
    } else{
